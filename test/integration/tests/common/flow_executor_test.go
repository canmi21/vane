...
func TestFlowTimeout(t *testing.T) {
	...
	bin, err := sb.CompileGoBin(executorPluginSrc)
	if err != nil {
		t.Fatal(err)
	}
	...
	sb.Env["FLOW_EXECUTION_TIMEOUT_SECS"] = "1"
	...
	// 3. Register Plugin
	if err := registerExecutorPlugin(sb.ConsolePort, token, bin, "sleep"); err != nil {
		t.Fatal(err)
	}
	...
	vanePort := ports[0]
	if err := writeExecutorFlow(sb, vanePort); err != nil {
		t.Fatal(err)
	}
	...
	if resp.StatusCode < 500 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected timeout error, got status %d", resp.StatusCode), nil))
	}

	if err := proc.WaitForLog("Flow execution timed out", 2*time.Second); err != nil {
		t.Fatal(term.FormatFailure("Vane did not log timeout message", term.NewNode(proc.DumpLogs())))
	}
}

func TestExternalCircuitBreaker(t *testing.T) {
	sb, ctx := env.SetupTest(t)
	debug, _ := ctx.Value(env.DebugKey).(bool)
	bin, err := sb.CompileGoBin(executorPluginSrc)
	if err != nil {
	...
	sb.Env["EXTERNAL_PLUGIN_QUIET_PERIOD_SECS"] = "2"
	...
	if err := registerExecutorPlugin(sb.ConsolePort, token, bin, "fail"); err != nil {
		t.Fatal(err)
	}
	...
	if err := proc.WaitForLog("Marking as failed in Circuit Breaker", 2*time.Second); err != nil {
		t.Fatal(term.FormatFailure("Vane did not activate circuit breaker", term.NewNode(proc.DumpLogs())))
	}

	// Req 2: Should hit Circuit Breaker immediately (Fast Fail to 'failure' branch)
	resp2, err := client.Get(url)
	if err != nil {
		t.Fatal(term.FormatFailure("Circuit breaker request failed", term.NewNode(err.Error())))
	}
	defer resp2.Body.Close()

	// Our flow maps 'failure' branch to 503
	if resp2.StatusCode != 503 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Circuit breaker did not return 503, got %d", resp2.StatusCode), nil))
	}

	body, _ := io.ReadAll(resp2.Body)
	if !bytes.Contains(body, []byte("Circuit Breaker Active")) {
		t.Fatal(term.FormatFailure("Wrong response body for circuit breaker", term.NewNode(string(body))))
	}
