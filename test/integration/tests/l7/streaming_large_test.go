...
func TestStreamH2toH3(t *testing.T) {
	sb, ctx := env.SetupTest(t)
	if err := RunStreamingTest(ctx, sb, ClientH2, UpstreamH3); err != nil {
		t.Fatal(err)
	}
}

func TestStreamH3toH2(t *testing.T) {
	sb, ctx := env.SetupTest(t)
	if err := RunStreamingTest(ctx, sb, ClientH3, UpstreamH2); err != nil {
		t.Fatal(err)
	}
}
