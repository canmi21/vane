...
func TestStaticServeBasic(t *testing.T) {
	...
	vanePort, _, _ := setupStaticTest(sb, nil)
	...
	if !strings.Contains(string(body), "Hello Vane") {
		t.Fatal(term.FormatFailure("Failed to serve index.html", term.NewNode(string(body))))
	}
	...
	if resp2.StatusCode != 200 || resp2.ContentLength != 100 {
		t.Fatal(term.FormatFailure("Failed to serve large.txt", nil))
	}
}
...
func TestStaticRange(t *testing.T) {
	...
	if resp.StatusCode != 206 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected 206, got %d", resp.StatusCode), nil))
	}
	body, _ := io.ReadAll(resp.Body)
	if len(body) != 10 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected 10 bytes, got %d", len(body)), nil))
	}
	// "ABCDEFGHIJ" repeated. 10-19 should be "ABCDEFGHIJ" (the second block)
	if string(body) != "ABCDEFGHIJ" {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Wrong range content: %s", string(body)), nil))
	}
}
...
func TestStaticSPA(t *testing.T) {
	...
	debug, _ := ctx.Value(env.DebugKey).(bool)
	// Enable SPA mode
	vanePort, _, _ := setupStaticTest(sb, map[string]interface{}{"spa": true})
	...
	if resp.StatusCode != 200 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("SPA: Expected 200, got %d", resp.StatusCode), nil))
	}
	body, _ := io.ReadAll(resp.Body)
	if !strings.Contains(string(body), "Hello Vane") {
		t.Fatal(term.FormatFailure("SPA: Failed to fallback to index.html", term.NewNode(string(body))))
	}
}
...
func TestStaticBrowse(t *testing.T) {
	...
	vanePort, root, _ := setupStaticTest(sb, map[string]interface{}{"browse": true})
	...
	if resp.StatusCode != 200 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Browse: Expected 200, got %d", resp.StatusCode), nil))
	}
	body, _ := io.ReadAll(resp.Body)
	// Check for a link to large.txt
	if !strings.Contains(string(body), "large.txt") || !strings.Contains(string(body), "<a href=") {
		t.Fatal(term.FormatFailure("Browse: Listing HTML missing file links", term.NewNode(string(body))))
	}
}
...
func TestStaticTraversal(t *testing.T) {
	...
	if resp.StatusCode == 200 {
		t.Fatal(term.FormatFailure("Directory traversal succeeded (vulnerability!)", nil))
	}
}
...
func TestStaticPrecompressed(t *testing.T) {
	...
	vanePort, root, _ := setupStaticTest(sb, map[string]interface{}{"precompress": true})
	...
	if resp.StatusCode != 200 {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected 200, got %d", resp.StatusCode), nil))
	}

	// Verify Header
	if ce := resp.Header.Get("Content-Encoding"); ce != "gzip" {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected Content-Encoding: gzip, got '%s'", ce), nil))
	}

	// Verify Body (Should be RAW gzip bytes because we disabled compression)
	body, _ := io.ReadAll(resp.Body)
	if !bytes.Equal(body, b.Bytes()) {
		t.Fatal(term.FormatFailure("Did not serve correct .gz bytes", nil))
	}
	...
	if ce := resp2.Header.Get("Content-Encoding"); ce != "" {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Expected no Content-Encoding, got '%s'", ce), nil))
	}
	// Verify Body (should serve original)
	body2, _ := io.ReadAll(resp2.Body)
	if string(body2) != "body { color: red; }" {
		t.Fatal(term.FormatFailure("Did not serve original content", term.NewNode(string(body2))))
	}
}
