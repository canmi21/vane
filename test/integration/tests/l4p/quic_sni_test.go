...
func TestQuicSniProxy(t *testing.T) {
	...
	if err != nil {
		t.Fatal(term.FormatFailure("QUIC Handshake Failed (SNI Match)",
	...
	if err == nil {
		t.Fatal(term.FormatFailure("Expected QUIC failure for wrong SNI, but got success", nil))
	}
}
