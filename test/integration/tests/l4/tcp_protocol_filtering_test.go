...
func TestTcpProtocolFiltering(t *testing.T) {
	...
	if err != nil {
		t.Fatal(term.FormatFailure("Failed to connect for HTTP test", nil))
	}
	...
	if err != nil || n == 0 {
		t.Fatal(term.FormatFailure("Valid HTTP was filtered out", nil))
	}
	...
	if err != nil {
		t.Fatal(term.FormatFailure("Failed to connect for TLS test", nil))
	}
	...
	if err == nil {
		t.Fatal(term.FormatFailure("Mismatching traffic (TLS) was NOT filtered out", nil))
	}
}
