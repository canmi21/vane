...
func TestBackendRecovery(t *testing.T) {
	...
	sb.Env["HEALTH_TCP_INTERVAL_SECS"] = "2"
	...
	if successCount == 0 {
		t.Fatal(term.FormatFailure("Failover failed: No successful requests after S1 down", nil))
	}
	...
	if newS1 == nil {
		t.Fatal(term.FormatFailure(fmt.Sprintf("Failed to restart S1 on port %d", s1Port), term.NewNode(err.Error())))
	}
	defer newS1.Close()
	...
	if err := proc.WaitForLog("has recovered", 1*time.Second); err != nil {
		t.Fatal(term.FormatFailure("Vane did not log recovery message", nil))
	}
}
