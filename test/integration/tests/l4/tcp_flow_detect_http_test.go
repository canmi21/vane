/* test/integration/tests/l4/tcp_flow_detect_http_test.go */
...
func TestTcpFlowDetectHttp(t *testing.T) {
	...
	if err := verifyTcpConnection(vanePort, "GET / HTTP/1.1\r\n\r\n", true); err != nil {
		t.Fatal(term.FormatFailure("Positive Check Failed (HTTP)", term.NewNode(err.Error())))
	}
