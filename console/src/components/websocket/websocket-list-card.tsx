/* src/components/websocket/websocket-list-card.tsx */

import { ChevronRight, Cable, HelpCircle, AppWindow } from "lucide-react";
...
export function WebSocketListCard({
	domains,
	selectedDomain,
	onSelectDomain,
}: {
		...
		<div className="w-full rounded-xl border border-[var(--color-bg-alt)] bg-[var(--color-bg)] shadow-sm">
			<div className="border-b border-[var(--color-bg-alt)] p-6">
				<div className="flex items-center gap-3">
					<Cable size={20} className="stroke-[var(--color-theme-border)]" />
					<h3 className="text-lg font-semibold text-[var(--color-text)]">
						WebSocket Proxy Policies
					</h3>
			...
			<div className="divide-y divide-[var(--color-bg-alt)]">
				{sortedDomains.length > 0 ? (
					...
					<div className="p-12 text-center text-[var(--color-subtext)]">
						...
						<p className="text-sm">
							Configure domains to set WebSocket policies.
						</p>
					</div>
				)}
			</div>
