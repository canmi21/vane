/* src/routes/$instance/error-pages/$page.tsx */
...
export const Route = createFileRoute("/$instance/error-pages/$page")({
	component: ErrorPagesDetailPage,
});

function ErrorPagesDetailPage() {
	const { instance: instanceId, page } = useParams({
		from: "/$instance/error-pages/$page",
	});
	// The "_" is a placeholder for the "no selection" or "empty" state.
	const selectedTemplate = page === "_" ? null : page;
	...
	>({
		...
		onSuccess: (_, vars) => {
			...
			handleTemplateSelect(vars.name);
		},
	});
	...
	>({
		...
		onSuccess: (_, vars) => {
			// Invalidate the specific content query to refetch fresh data for the editor
			queryClient.invalidateQueries({
				queryKey: ["instance", instanceId, "templates", "content", vars.name],
			});
		},
	});
	...
	const removeMutation = useMutation<RequestResult<unknown>, Error, string>({
		...
		onSuccess: (_, deletedName) => {
			...
			if (selectedTemplate === deletedName) {
