/* src/routes/$instance/certificates/$domain.tsx */
...
export const Route = createFileRoute("/$instance/certificates/$domain")({
	component: CertificatesDetailPage,
});

function CertificatesDetailPage() {
	const { instance: instanceId, domain } = useParams({
		from: "/$instance/certificates/$domain",
	});
	...
	// --- FIX: Added custom sorting to push wildcard domains to the end ---
	const certDomains = useMemo(() => {
		return Object.keys(certificates).sort((a, b) => {
	...
	const certListData = useMemo<CertListItem[]>(() => {
		// This will now use the sorted certDomains list automatically
	...
	const removeMutation = useMutation<RequestResult<unknown>, Error, string>({
		mutationFn: (domainToDelete) => deleteCert(instanceId, domainToDelete),
