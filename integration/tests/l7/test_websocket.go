...
func TestWSDeny(ctx context.Context, s *env.Sandbox) error {
	...
	if err == nil {
		...
		conn.Close()
		return term.FormatFailure("WebSocket connection succeeded but should be denied", nil)
	}
	...
	return term.FormatFailure("Unexpected error type", term.NewNode(errStr))
}
...
func TestWSAllow(ctx context.Context, s *env.Sandbox) error {
	...
	if string(buf) != msg {
		return term.FormatFailure("Echo mismatch", term.NewNode(fmt.Sprintf("Expected: %s, Got: %s", msg, string(buf))))
	}
...
func TestWSStreamLarge(ctx context.Context, s *env.Sandbox) error {
	...
	case <-time.After(120 * time.Second):
		return term.FormatFailure("Timeout after 120s", nil)
	}
