/* integration/tests/l7/test_external_generic.go */
package l7
...
func TestExternalApiRegistration(ctx context.Context, s *env.Sandbox) error {
	...
	binPath, err := s.CompileGoBin(pluginGoSource)
	if err != nil {
		return term.FormatFailure("Plugin Compilation Failed", term.NewNode(err.Error()))
	}
	...
	if err := registerPlugin(s.ConsolePort, token, binPath); err != nil {
		return term.FormatFailure("API Registration Failed", term.NewNode(err.Error()))
	}

	// 4. Configure Listener (using hot-reload)
	ports, _ := env.GetFreePorts(1)
	vanePort := ports[0]
	if err := writeListenerConfig(s, vanePort, upstream.Port); err != nil {
		return err
	}
	...
	if err := verifyTraffic(vanePort); err != nil {
		return term.FormatFailure("Traffic Verification Failed", term.NewNode(err.Error()))
	}
...
func TestExternalPersistence(ctx context.Context, s *env.Sandbox) error {
	debug, _ := ctx.Value(env.DebugKey).(bool)
	...
	s.Env["ACCESS_TOKEN"] = token
	proc1, err := s.StartVane(ctx, debug)
	if err != nil {
		return err
	}
	...
	if err := registerPlugin(s.ConsolePort, token, binPath); err != nil {
		proc1.Stop()
		return term.FormatFailure("API Registration Failed (Run 1)", term.NewNode(err.Error()))
	}
	...
	proc2, err := s.StartVane(ctx, debug)
	if err != nil {
		return err
	}
	defer proc2.Stop()
	...
	if err := proc2.WaitForTcpPort(vanePort, 5*time.Second); err != nil {
		return term.FormatFailure("Port failed to start (Run 2)", term.NewNode(err.Error()))
	}

	// 7. Verify Traffic
	if err := verifyTraffic(vanePort); err != nil {
		return term.FormatFailure("Traffic Verification Failed (Run 2)", term.NewNode(err.Error()))
	}
