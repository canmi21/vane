/* integration/tests/l4p/test_tls_alpn.go */
package l4p
...
func TestTlsAlpnProxy(ctx context.Context, s *env.Sandbox) error {
	...
	if err != nil {
		return term.FormatFailure("ALPN Match Failed", term.NewNode(err.Error()))
	}

	// Verify ConnectionState negotiation
	state := conn.ConnectionState()
	if state.NegotiatedProtocol != "h2" {
		return term.FormatFailure("ALPN Negotiation failed", term.NewNode(fmt.Sprintf("Got: %s", state.NegotiatedProtocol)))
	}
	...
	conn.Close()
	if resp != "alpn-test\n" {
		return term.FormatFailure("Data echo failed", nil)
	}
	...
	if err == nil {
		return term.FormatFailure("Expected failure for wrong ALPN", nil)
	}
