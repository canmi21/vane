...
func TestTlsSniStream(t *testing.T) {
	...
	if err != nil {
		t.Fatal(term.FormatFailure("TLS Handshake Failed", term.NewNode(err.Error())))
	}
	defer conn.Close()
	...
	for i := 0; i < 5; i++ {
		...
		if _, err := conn.Write([]byte(payload)); err != nil {
			t.Fatal(term.FormatFailure(fmt.Sprintf("Write failed at iter %d", i), term.NewNode(err.Error())))
		}

		line, err := reader.ReadString('\n')
		if err != nil {
			t.Fatal(term.FormatFailure(fmt.Sprintf("Read failed at iter %d", i), term.NewNode(err.Error())))
		}

		if line != payload {
			t.Fatal(term.FormatFailure("Data corruption", term.NewNode(fmt.Sprintf("Sent: %q, Got: %q", payload, line))))
		}
	}
	...
	if err == nil {
		...
		t.Fatal(term.FormatFailure("Expected TLS Handshake failure for wrong SNI", nil))
	}
}
