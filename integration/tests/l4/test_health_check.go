/* integration/tests/l4/test_health_check.go */
package l4
...
func TestTcpHealthCheck(ctx context.Context, s *env.Sandbox) error {
	...
	s.Env["HEALTH_TCP_INTERVAL_SECS"] = "1"
	...
	if err == nil {
		...
		if err == nil {
			return term.FormatFailure("Got data when backend is down", nil)
		}
	...
	if err != nil {
		return term.FormatFailure("Failed to restart backend", term.NewNode(err.Error()))
	}
	...
	if !recovered {
		return term.FormatFailure("Failed to recover after backend restart", term.NewNode(proc.DumpLogs()))
	}
