/* integration/tests/l4/test_target_resolution.go */
package l4
...
func TestResolveIp(ctx context.Context, s *env.Sandbox) error {
	...
	s.WriteConfig(fmt.Sprintf("listener/[%d]/tcp.yaml", vanePort), bytes)
	...
	if err != nil {
		return term.FormatFailure("Failed to connect via IP", term.NewNode(err.Error()))
	}
	conn.Close()
	return nil
}

func TestResolveNode(ctx context.Context, s *env.Sandbox) error {
	debug, _ := ctx.Value(env.DebugKey).(bool)
	...
	s.WriteConfig("nodes.yml", nodeBytes)
	...
	conn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", vanePort), 1*time.Second)
	if err != nil {
		return term.FormatFailure("Failed to connect via Node", term.NewNode(err.Error()))
	}
	conn.Close()
	return nil
}

func TestResolveDomain(ctx context.Context, s *env.Sandbox) error {
	debug, _ := ctx.Value(env.DebugKey).(bool)
	...
	conn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", vanePort), 1*time.Second)
	if err != nil {
		return term.FormatFailure("Failed to connect via Domain", term.NewNode(err.Error()))
	}
	conn.Close()
	return nil
}
...
func TestResolveDomainFailure(ctx context.Context, s *env.Sandbox) error {
	...
	if err == nil {
		...
		conn.Close()
		if err == nil {
			return term.FormatFailure("Traffic should have been blocked (DNS Fail)", nil)
		}
	}
