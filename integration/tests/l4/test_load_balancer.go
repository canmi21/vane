/* integration/tests/l4/test_load_balancer.go */
package l4
...
func TestLoadBalancerRandom(ctx context.Context, s *env.Sandbox) error {
	...
	if err := s.WriteConfig(fmt.Sprintf("listener/[%d]/tcp.yaml", vanePort), confBytes); err != nil {
		return err
	}
	...
	if !stabilized {
		return term.FormatFailure("Failed to stabilize load balancer pool", term.NewNode(fmt.Sprintf("Only subset active.\nLogs:\n%s", proc.DumpLogs())))
	}
	...
	for i := 0; i < requestCount; i++ {
		conn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", vanePort), 500*time.Millisecond)
		if err != nil {
			return term.FormatFailure("Failed to connect to LB", term.NewNode(err.Error()))
		}
		conn.Write([]byte("ping\n"))
		conn.Close()
	}
	...
	for i, srv := range healthyServers {
		...
		total += c
		if c < 50 || c > 150 {
			return term.FormatFailure(fmt.Sprintf("Server %d load unbalanced", i), term.NewNode(fmt.Sprintf("Count: %d (Expected ~100)", c)))
		}
	}
...
func TestLoadBalancerSerial(ctx context.Context, s *env.Sandbox) error {
	debug, _ := ctx.Value(env.DebugKey).(bool)
	...
	if !stabilized {
		return term.FormatFailure("Failed to stabilize load balancer pool", term.NewNode(fmt.Sprintf("Only subset of backends active after 10s.\nLogs:\n%s", proc.DumpLogs())))
	}
	...
	for i := 0; i < requestCount; i++ {
		conn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", vanePort), 500*time.Millisecond)
		if err != nil {
			return term.FormatFailure("Failed to connect", term.NewNode(err.Error()))
		}
		conn.Write([]byte("ping\n"))
		conn.Close()
	...
	for i, srv := range healthyServers {
		...
		if increment != 10 {
			return term.FormatFailure(fmt.Sprintf("Server %d count mismatch (Serial)", i), term.NewNode(fmt.Sprintf("Got +%d, Expected +10 (Total: %d)", increment, current)))
		}
	}
