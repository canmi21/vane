/* integration/tests/l4/test_tcp_ip_filtering.go */
package l4
...
func TestTcpIpFiltering(ctx context.Context, s *env.Sandbox) error {
	...
	if err := s.WriteConfig(fmt.Sprintf("listener/[%d]/tcp.json", vanePort), genConfig("1.2.3.4")); err != nil {
		return err
	}
	...
	// Update Config
	if err := s.WriteConfig(fmt.Sprintf("listener/[%d]/tcp.json", vanePort), genConfig("127.0.0.1")); err != nil {
		return err
	}

	// Wait for reload signal explicitly
	if err := proc.WaitForLog("Config change signal", 5*time.Second); err != nil {
		return term.FormatFailure("Vane did not reload config", term.NewNode(err.Error()))
	}
	...
	for {
		...
		if time.Now().After(retryDeadline) {
			return term.FormatFailure("Failed to connect for Allow test after reload", term.NewNode(err.Error()+"\n\n--- Vane Logs ---\n"+proc.DumpLogs()))
		}
	...
	if err != nil || resp != "ping\n" {
		return term.FormatFailure("Traffic denied/broken when IP should be allowed", nil)
	}
