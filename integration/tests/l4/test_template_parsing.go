/* integration/tests/l4/test_template_parsing.go */
package l4
...
func TestTemplateParsing(ctx context.Context, s *env.Sandbox) error {
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to compile validator plugin: %v\nOutput: %s", err, string(out))
	}
	...
	resultFile := filepath.Join(s.RootDir, "validator_result.txt")
	apiURL := fmt.Sprintf("http://127.0.0.1:%d/plugins/test.validator", s.ConsolePort)
	...
	if err != nil {
		return fmt.Errorf("failed to call register plugin API: %v", err)
	}
	...
	if err := s.WriteConfig(fmt.Sprintf("listener/[%d]/tcp.json", vanePort), flowBytes); err != nil {
		return err
	}

	// Wait for listener to be UP
	if err := proc.WaitForTcpPort(vanePort, 5*time.Second); err != nil {
		return term.FormatFailure("Port failed to start after config reload", term.NewNode(err.Error()))
	}
		...
		case <-ticker.C:
			...
			if err == nil {
				...
				if strings.HasPrefix(sContent, "FAILURE") {
					return term.FormatFailure(fmt.Sprintf("Validator reported failure: %s", sContent), nil)
				}
			}
		case <-time.After(time.Until(deadline)):
			return term.FormatFailure("Timeout waiting for validator result file", nil)
		}
	}
}
