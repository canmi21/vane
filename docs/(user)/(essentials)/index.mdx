...
Your routing logic cannot be fully expressed in static configuration files. Instead, it depends on real-time external state—such as service discovery backends, feature flags, database queries, or custom business logic. Vane allows you to query external systems, make routing decisions programmatically, and update traffic behavior without restarting or reloading the proxy.
</Accordion>
