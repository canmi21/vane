...
**Crucially**, it verifies that the resolved path is strictly within the `CONFIG_DIR`. This prevents directory traversal attacks where a malicious configuration path might point to sensitive system files.
...
If type `T` implements the `PreProcess` trait, the `pre_process()` hook is called. This allows configurations to mutate themselves immediately after loading (e.g., setting default inner values or injecting context like the filename).
...
Finally, the `validate()` method (from the `validator` crate) is executed. If validation fails (e.g., a required field is missing or a port is out of range), the loader returns `None` and logs a detailed error.
