...
- **Partial Parsing**: It parses _just enough_ to route (e.g., ClientHello), then tunnels or hands off.
...
<Step>
	### Upgrade Signal The L4 flow executes an **Upgrade Terminator** (e.g., `upgrade "tls"`). The
	engine suspends L4 processing and wraps the stream in a TLS parser.
</Step>

<Step>
	### L4+ Routing The **L4+ Flow** (`tls.json`) parses the SNI (e.g., `example.com`). It decides
	this domain requires WAF processing.
</Step>
