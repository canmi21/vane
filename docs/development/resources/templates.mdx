...
<Mermaid
  chart='
graph TD;
    subgraph Step1 [Phase 1: Parse Inner Variable]
        Inner["{{conn.protocol}}"] -- Lookup --> Value1["http"]
    end

    subgraph Step2 [Phase 2: Construct Outer Key]
        Value1 --> Concat["kv. + http + _backend"]
        Concat --> Key["kv.http_backend"]
    end

    subgraph Step3 [Phase 3: Final Resolution]
        Key -- Lookup --> Final["backend-01"]
    end
...
- **Limit**: To prevent ReDoS or stack overflow, parsing is limited to a depth of 5 and 50 total nodes by default.
...
The resolved key is passed to a `TemplateContext`. While usually linked to the connection's `KvStore`, the context can also "hijack" data directly from protocol streams (e.g., peeking at request bodies or headers on-demand).
