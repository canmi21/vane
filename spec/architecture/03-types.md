...
- `http_version` uses `OnceLock` — set exactly once before L7 dispatch starts. On a TLS listener the version is decided by the negotiated ALPN at handshake time (`http/1.1` vs `h2`) and the listener seeds the field immediately after the handshake; on a cleartext listener (no ALPN) the H1/H2 driver seeds it during the L4→L7 upgrade. Readers (middleware, fetches, predicates) observe `None` until that single set. `OnceLock::set` is idempotent on the second call — both code paths are written defensively so a TLS-listener seed followed by a driver-seed is a no-op rather than a panic.
