...
| S1-05  | Middleware + Fetch traits (`L4Peek`/`L4Bytes`/`L7Req`/`L7Resp`/`L7Fetch`/`L4Fetch`, `Decision`, `Tunnel`)                                       | `trait-variant` 0.1                                               | S1-04               |
| S1-06  | `PredicateInst` + `CompiledOperator`/`CompiledValue` + `PredicateView`                                                                          | `fancy-regex`, `ipnet`                                            | S1-02, S1-04        |
| S1-07  | Rule JSON parse (`Predicate` untagged, `CheckMap` custom deserialize, `RawRule`, `MiddlewareRef`, `FetchSpec`, `SourceInfo`)                    | `serde`, `serde_json`                                             | S1-06               |
...
| S1-25  | `vane-mgmt` MVP verbs: `compile_dry_run`, `reload`, `get_active_config`, `stats`, `shutdown`, `list_connections`                                | —                                                                 | S1-24, S1-12        |
...
| S2-15 | Streaming mgmt verbs: `tail_flow_log`, `tail_log`                                                                                                 | `tokio::sync::broadcast`                           | S1-29, S2-14        |
...
| S2-20 | `vane` CLI: `tail`, `stats`, `list-connections`, `get-metrics`                                                                                    | `vane-mgmt` client                                 | S1-32, S2-15, S2-16 |
...
| S3-08 | `L7Fetch::Cgi` via `HttpUpstream::Cgi` (`tokio::process::Command` + `CommandExt::pre_exec` for setuid/gid/rlimits) | `tokio::process`, `libc`                                                  | S2-17        |
...
| S3-15 | `list_wasm_pools` + `list_upstreams` mgmt verbs                                                                    | —                                                                         | S2-15, S3-05 |
