...
| `account_key_path` | string                         | no                     | (auto-create+persist) | BYO account key. Mutually exclusive with auto-create.    |
| `key_type`         | `"ecdsa-p256"` \| `"rsa-2048"` | no                     | `ecdsa-p256`          | Subject key algorithm.                                   |
| `renew_before`     | duration                       | no                     | `"30d"`               | Same duration grammar as `rate_limit.window`.            |
| `san`              | list\<string\>                 | no                     | `[tls.sni]`           | Subject Alternative Names included in the issued cert.   |
...
- `account_key_path` set together with any future explicit auto-create flag → error (auto-create is the absence of the path).
