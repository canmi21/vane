/* src/modules/plugins/protocol/tls/sni.rs */
...
pub struct TlsSniPlugin;

impl Plugin for TlsSniPlugin {
	fn name(&self) -> &'static str {
		"internal.protocol.tls.sni"
	}
...
#[async_trait]
impl Middleware for TlsSniPlugin {
	...
	async fn execute(&self, inputs: ResolvedInputs) -> Result<MiddlewareOutput> {
		...
		let extracted = clienthello::extract_sni(&payload)?;

		let branch = if let Some(sni) = extracted {
			if sni == target_sni { "true" } else { "false" }
		} else {
			"false"
		};
