			...
			Router::new()
				...
				.nest(
					...
					Router::new()
						...
						.route("/{port}/{protocol}/flow/validate", post(flow::validate_flow_handler)),
				)
				.nest(
					"/plugins",
					Router::new().route("/", get(plugins::list_plugins_handler)).route(
						"/{name}",
				...
				.nest(
					"/nodes",
					Router::new()
						.route("/", get(nodes::list_nodes_handler).post(nodes::create_node_handler))
						.route(
							"/{name}",
				...
				.nest(
					"/certs",
					Router::new().route("/", get(certs::list_certs_handler)).route(
						"/{id}",
				...
				.nest(
					"/resolvers",
					Router::new().route("/", get(resolvers::list_resolvers_handler)).route(
						"/{protocol}",
				...
				.nest(
					"/applications",
					Router::new().route("/", get(applications::list_applications_handler)).route(
						"/{protocol}",
