		...
		loop {
			tokio::select! {
				Ok((socket, addr)) = listener.accept() => {
					...
					let Some(_guard) = vane_primitives::tasks::GLOBAL_TRACKER.acquire(client_ip) else {
					...
					let tcp_config = vane_engine::config::get().listeners.get_tcp(&port.to_string());
