...
use crate::config::{FlowNode, TerminationAction};
...
pub async fn execute(
	...
	timeout: Duration,
) -> Result<TerminationAction, FlowError> {
...
) -> Pin<Box<dyn Future<Output = Result<TerminationAction, FlowError>> + Send + 'a>> {
	Box::pin(async move {
		let plugin = registry
			.get(&node.plugin)
		...
		match plugin {
			PluginAction::Middleware(mw) => {
				...
				let next = node.branches.get(&action.branch).ok_or_else(|| FlowError::BranchNotFound {
			...
			PluginAction::Terminator(term) => {
				...
				term
					.execute(&node.params, context.kv(), stream, context.peer_addr(), context.server_addr())
					.await
					.map_err(|source| FlowError::PluginFailed { name: node.plugin.clone(), source })?;
...
mod tests {
	use super::*;
	use crate::config::{FlowNode, Layer, TerminationAction};
	...
	async fn single_terminator_step() {
		...
		assert_eq!(result.unwrap(), TerminationAction::Finished);
	...
	async fn middleware_then_terminator() {
		...
		let registry = PluginRegistry::new()
			.register(
				"mock.branch",
				PluginAction::Middleware(Box::new(FixedBranchMiddleware { branch: "next".to_owned() })),
			)
			.register(
				"mock.term",
				PluginAction::Terminator(Box::new(MockTerminator { called: called.clone() })),
	...
	async fn missing_branch_error() {
		let registry = PluginRegistry::new().register(
			"mock.branch",
			PluginAction::Middleware(Box::new(FixedBranchMiddleware { branch: "missing".to_owned() })),
		);
	...
	async fn timeout_triggers() {
		let registry =
			PluginRegistry::new().register("slow", PluginAction::Terminator(Box::new(SlowTerminator)));
		...
		let result = execute(&node, &mut ctx, &registry, Duration::from_millis(50)).await;
	...
	async fn stream_already_consumed() {
		...
		let result = execute(&node, &mut ctx, &registry, Duration::from_secs(5)).await;
		assert!(matches!(result, Err(FlowError::StreamAlreadyConsumed { .. })));
	}
