...
impl TcpForward {
	fn resolve_target(
		...
		if let (Some(ip), Some(port)) = (
			...
			return Ok(ResolvedTarget { addr: SocketAddr::new(ip, port) });
		}

		// Fall back to KV store
		if let (Some(ip_str), Some(port_str)) = (kv.get("target.ip"), kv.get("target.port")) {
			let ip: IpAddr =
				ip_str.parse().map_err(|e| anyhow::anyhow!("invalid target.ip in kv: {e}"))?;
			let port: u16 =
				port_str.parse().map_err(|e| anyhow::anyhow!("invalid target.port in kv: {e}"))?;
			return Ok(ResolvedTarget { addr: SocketAddr::new(ip, port) });
		}

		Err(anyhow::anyhow!("no target found: set params.ip+params.port or kv target.ip+target.port"))
	}
}
...
mod tests {
	...
	fn resolve_from_kv() {
		...
		let result = TcpForward::resolve_target(&serde_json::Value::Null, &kv).unwrap();
		assert_eq!(result.addr, "172.16.0.1:9090".parse::<SocketAddr>().unwrap());
	}
