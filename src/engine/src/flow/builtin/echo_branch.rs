...
impl Middleware for EchoBranch {
	fn execute(
		...
		let branch =
			params.get("branch").and_then(serde_json::Value::as_str).unwrap_or("default").to_owned();

		let key =
			params.get("key").and_then(serde_json::Value::as_str).unwrap_or("echo.visited").to_owned();

		let value =
			params.get("value").and_then(serde_json::Value::as_str).unwrap_or("true").to_owned();

		Ok(BranchAction { branch, updates: vec![(key, value)] })
	}
}
...
mod tests {
	...
	fn default_params() {
		...
		let result = EchoBranch.execute(&serde_json::Value::Null, &ctx).expect("should succeed");
	...
	fn custom_params() {
		...
		let result = EchoBranch.execute(&params, &ctx).expect("should succeed");
