...
impl Middleware for ProtocolDetect {
	fn execute(
		...
		let data =
			ctx.peek_data().ok_or_else(|| anyhow::anyhow!("no peek data for protocol detection"))?;
...
pub fn default_rules() -> Vec<DetectRule> {
	...
	for content_type in [0x14, 0x15, 0x16, 0x17] {
		rules.push(DetectRule {
			...
			condition: MatchCondition::MagicBytes { offset: 0, bytes: vec![content_type, 0x03] },
		});
	}
	...
	// Catch-all
	rules.push(DetectRule { protocol: "unknown".to_owned(), condition: MatchCondition::Fallback });
...
mod tests {
	...
	fn detect_with_defaults(data: &[u8]) -> Result<String, anyhow::Error> {
		...
		pd.execute(&serde_json::Value::Null, &ctx).map(|a| a.branch)
	}
	...
	#[test]
	fn http_get() {
		assert_eq!(detect_with_defaults(b"GET / HTTP/1.1\r\n").unwrap(), "http");
	}

	#[test]
	fn http_post() {
		assert_eq!(detect_with_defaults(b"POST /api HTTP/1.1\r\n").unwrap(), "http");
	}

	#[test]
	fn http2_preface() {
		assert_eq!(detect_with_defaults(b"PRI * HTTP/2.0\r\n").unwrap(), "http");
	}
	...
	fn no_fallback_rule_and_random_bytes_returns_error() {
		let rules = vec![DetectRule {
			protocol: "tls".to_owned(),
			condition: MatchCondition::MagicBytes { offset: 0, bytes: vec![0x16, 0x03] },
		}];
	...
	fn first_match_wins() {
		let rules = vec![
			DetectRule { protocol: "first".to_owned(), condition: MatchCondition::Fallback },
			DetectRule { protocol: "second".to_owned(), condition: MatchCondition::Fallback },
		];
		let pd = ProtocolDetect::new(rules);
		let ctx = MockContext::with_peek(&[0x00]);
		assert_eq!(pd.execute(&serde_json::Value::Null, &ctx).unwrap().branch, "first");
	}
	...
	fn magic_bytes_with_offset() {
		let rules = vec![DetectRule {
			...
			condition: MatchCondition::MagicBytes { offset: 2, bytes: vec![0xAB, 0xCD] },
		}];
		let pd = ProtocolDetect::new(rules);
		let ctx = MockContext::with_peek(&[0x00, 0x00, 0xAB, 0xCD, 0xEF]);
		assert_eq!(pd.execute(&serde_json::Value::Null, &ctx).unwrap().branch, "custom");
	}
	...
	fn data_shorter_than_pattern() {
		let cond = MatchCondition::MagicBytes { offset: 0, bytes: vec![0x16, 0x03, 0x01] };
	...
	fn offset_beyond_data_length() {
		let cond = MatchCondition::MagicBytes { offset: 10, bytes: vec![0x01] };
