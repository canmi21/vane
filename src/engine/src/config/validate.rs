            ...
            if let Some(l5) = &port_config.l5 {
                if !self.certs.contains_key(&l5.default_cert) {
                    errors.push(ValidationError {
                        ...
                        step_path: vec![],
                        message: format!(
                            "default_cert {:?} not found in certs map",
                            l5.default_cert
                        ),
                    });
                }
...
mod tests {
    ...
    fn valid_single_layer() {
        let l4 = simple_middleware(HashMap::from([
            ("default".to_owned(), simple_terminator()),
        ]));
        let config = simple_config(HashMap::from([
            (80, simple_port_l4_only(l4)),
        ]));
    ...
    fn valid_multi_layer() {
        ...
        l4_term.termination = Some(TerminationAction::Upgrade {
            target_layer: Layer::L5,
        });

        let l4 = simple_middleware(HashMap::from([
            ("default".to_owned(), l4_term),
        ]));

        let mut l5_term = simple_terminator();
        l5_term.termination = Some(TerminationAction::Upgrade {
            target_layer: Layer::L7,
        });
    ...
    fn upgrade_l5_missing_config() {
        ...
        term.termination = Some(TerminationAction::Upgrade {
            target_layer: Layer::L5,
        });

        let l4 = simple_middleware(HashMap::from([
            ("default".to_owned(), term),
        ]));
        // No l5 config
        let config = simple_config(HashMap::from([
            (80, simple_port_l4_only(l4)),
    ...
    fn merge_then_validate() {
        ...
        let config = simple_config(HashMap::from([
            (80, simple_port_l4_only(l4.clone())),
        ]));

        let patch = ConfigPatch {
            ports: Some(HashMap::from([
                (443, simple_port_l4_only(l4)),
            ])),
            global: None,
            certs: None,
        };
    ...
    fn l5_cert_missing() {
        ...
        let l5 = L5Config {
            default_cert: "missing_cert".to_owned(),
            alpn: vec![],
            flow: simple_terminator(),
        };
    ...
    fn terminator_with_branches() {
        let l4 = FlowNode {
            ...
            params: serde_json::json!({"ip": "127.0.0.1", "port": 8080}),
            branches: HashMap::from([
                ("bad".to_owned(), simple_terminator()),
            ]),
            termination: Some(TerminationAction::Finished),
        };
        let config = simple_config(HashMap::from([
    ...
    fn l7_upgrade_forbidden() {
        ...
        l7_term.termination = Some(TerminationAction::Upgrade {
            target_layer: Layer::L7,
        });
