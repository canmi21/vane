...
const fn default_flow_timeout_ms() -> u64 {
	10_000
}
...
impl Default for ListenConfig {
	fn default() -> Self {
		Self { address: default_address(), ipv6: false }
	}
}
...
mod tests {
	...
	fn sample_config() -> ConfigTable {
		let l4 = sample_middleware(HashMap::from([("default".to_owned(), sample_terminator())]));
		...
		let l7 = L7Config { flow: sample_terminator() };
	...
	fn cert_entry_pem_variant() {
		let entry = CertEntry::Pem { cert_pem: "CERT".to_owned(), key_pem: "KEY".to_owned() };
	...
	fn termination_action_serde() {
		let upgrade = TerminationAction::Upgrade { target_layer: Layer::L5 };
