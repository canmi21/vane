# tests/units/test_udp_single_client_burst.py
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    """
    Tests that Vane can correctly proxy a burst of UDP packets sent from a
    single client socket without packet loss.
    """
    ...
    try:
        ...
        # A standard DNS query is used as the payload.
        DNS_QUERY = (
...
"""
        ...
        with vane:
            ...
            # --- Send Traffic from a Single Client ---
            # A single socket is created outside the loop to simulate one client.
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as client:
                for _ in range(TOTAL_PACKETS):
                    client.sendto(DNS_QUERY, ("127.0.0.1", vane_port))
                    # This tiny sleep is critical. It paces the client's sends,
                    # preventing the OS network buffer for this single socket
                    # from overflowing, which would cause silent packet loss.
                    time.sleep(0.01)

            # This final sleep ensures Vane has time to process the *last*
            # packet before the context manager terminates the process.
            time.sleep(0.5)
        ...
        if packets_received != TOTAL_PACKETS:
            reason = (
                f"Backend did not receive the correct number of UDP packets from a single client.\n"
                f"      \n"
                f"      ├─ Test Scenario\n"
