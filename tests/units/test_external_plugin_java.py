# tests/units/test_external_plugin_java.py
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    """Test external plugin execution for: Java (JVM)"""
    http_server = None
    try:
        project_root = pathlib.Path(__file__).parent.parent.parent.resolve()
        plugins_dir = project_root / "examples/plugins/exec"
        class_file = plugins_dir / "test_java_template.class"
        interpreter = "java"

        if not shutil.which(interpreter):
            return (True, f"  ⚠ SKIPPED: '{interpreter}' not found.")
        if not class_file.exists():
            return (True, f"  ⚠ SKIPPED: Class file not found (needs compilation).")

        driver_config = {
            "type": "command",
            "program": interpreter,
            "args": ["-cp", str(plugins_dir), "test_java_template"],
            "env": {},
        }
        ...
        with vane:
            ...
            return lifecycle_test(
                vane, api_port, backend_port, "test_java_jvm", driver_config, debug_mode
            )
