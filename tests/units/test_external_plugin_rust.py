# tests/units/test_external_plugin_rust.py
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    """Test external plugin execution for: Rust (Binary)"""
    http_server = None
    try:
        project_root = pathlib.Path(__file__).parent.parent.parent.resolve()
        bin_path = project_root / "examples/plugins/exec/test_rust_template"

        if not bin_path.exists():
            return (True, f"  ⚠ SKIPPED: Binary not found: {bin_path.name}")
        ...
        with vane:
            ...
            return lifecycle_test(
                vane, api_port, backend_port, "test_rust_bin", driver_config, debug_mode
            )
