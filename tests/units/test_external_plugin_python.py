# tests/units/test_external_plugin_python.py
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    """Test external plugin execution for: Python"""
    http_server = None
    try:
        project_root = pathlib.Path(__file__).parent.parent.parent.resolve()
        script_path = project_root / "examples/plugins/exec/test_python_template.py"
        interpreter = "python3"
        ...
        with vane:
            ...
            return lifecycle_test(
                vane, api_port, backend_port, "test_python", driver_config, debug_mode
            )
