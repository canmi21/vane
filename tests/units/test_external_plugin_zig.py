# tests/units/test_external_plugin_zig.py
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    """Test external plugin execution for: Zig (Binary)"""
    http_server = None
    try:
        project_root = pathlib.Path(__file__).parent.parent.parent.resolve()
        bin_path = project_root / "examples/plugins/exec/test_zig_template"
        ...
        with vane:
            ...
            return lifecycle_test(
                vane, api_port, backend_port, "test_zig_bin", driver_config, debug_mode
            )
