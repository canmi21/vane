...
# --- Generic TOML for simple TCP tests ---
TOML_HTTP_SIMPLE = """
[[protocols]]
name = "http"
priority = 2
...
{
  "protocols": [
    { "name": "tls", "priority": 1, "detect": { "method": "magic", "pattern": "0x16" }, "session": { "keepalive": true, "timeout": 30000 }, "destination": { "type": "resolver", "resolver": "tls" } },
    { "name": "http", "priority": 2, "detect": { "method": "prefix", "pattern": "GET " }, "destination": { "type": "forward", "forward": { "strategy": "serial", "targets": [ { "ip": "127.0.0.1", "port": 8080 }, { "ip": "127.0.0.1", "port": 8081 } ], "fallbacks": [ { "ip": "127.0.0.1", "port": 8082 }, { "ip": "127.0.0.1", "port": 8083 } ] } } }
  ]
}
"""
TOML_TCP = """
...
destination = { type = "forward", forward = { strategy = "serial", targets = [ { ip = "127.0.0.1", port = 8080 }, { ip = "127.0.0.1", port = 8081 }, ], fallbacks = [ { ip = "127.0.0.1", port = 8082 }, { ip = "127.0.0.1", port = 8083 }, ] } }
"""
YAML_TCP = """
protocols:
  - name: tls
...
JSON_UDP = r"""
{ "protocols": [ { "name": "quic", "priority": 1, "detect": { "method": "magic", "pattern": "0xc3" }, "destination": { "type": "resolver", "resolver": "quic" } }, { "name": "dns", "priority": 2, "detect": { "method": "prefix", "pattern": "\u0000\u0001" }, "destination": { "type": "forward", "forward": { "strategy": "random", "targets": [ { "ip": "127.0.0.1", "port": 5353 }, { "ip": "127.0.0.1", "port": 5354 } ], "fallbacks": [ { "ip": "127.0.0.1", "port": 5355 }, { "ip": "127.0.0.1", "port": 5356 } ] } } } ] }
"""
TOML_UDP = r"""
[[protocols]]
name = "quic"
...
destination = { type = "forward", forward = { strategy = "random", targets = [ { ip = "127.0.0.1", port = 5353 }, { ip = "127.0.0.1", port = 5354 }, ], fallbacks = [ { ip = "127.0.0.1", port = 5355 }, { ip = "127.0.0.1", port = 5356 }, ] } }
"""
YAML_UDP = r"""
protocols:
  - name: quic
  ...
  - name: dns
    priority: 2
    detect: {method: prefix, pattern: "\0\x01"}
    destination:
      type: forward
      forward:
        strategy: random
