...
from utils.port_config_utils import (
    TOML_CONTENT,
    UP_STRING,
...
def run(debug_mode: bool) -> Tuple[bool, str]:
    ...
    try:
        ...
        with VaneInstance(env_vars, "", debug_mode) as vane:
            ...
            if not wait_for_log(vane, UP_STRING, 10, start_index=log_len_after_startup):
                log_dump = "".join(vane.captured_output)
                reason = f"Config file created, but did not find '{UP_STRING}' log."
                return (
                    False,
                    f"  └─ Details: {reason}\n\n--- Captured Log ---\n{log_dump}",
                )
