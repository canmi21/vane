...
//!
//! This commit lands the schema parser + bootstrap validation. Args
//! parsing is strict: every field listed in spec § _Full
//! `HttpUpstream::Cgi` shape_ is schema-required (no implicit defaults),
//! and the user-supplied `env` map is rejected for any reserved key
//! (RFC 3875 / common-extension / `HTTP_*`). The runtime fork-exec
//! path is filled in by the next commit; today the `L7Fetch` impl
//! returns an `Internal` error so any rule that links is caught the
//! moment a request lands rather than at link time.
//!
//! Auditor: Canmi
...
//
// `dead_code` on the leaf fields is silenced until the next commit's
// runtime path consumes them. Schema validation lands in this commit;
// fork-exec / env build / IO is the next.
#[allow(dead_code)]
#[derive(Debug, Clone)]
...
impl L7Fetch for CgiFetch {
	async fn fetch(
		...
		// Stub until the runtime lands in the next commit. Surface as
		// a 502 to the client (the executor's `finish_error` path
		// converts `Error::internal` to a 500; CGI semantics are 502
		// for "child failed", which the runtime commit will use too).
