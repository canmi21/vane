...
mod tests {
	...
	// Smoke test: loading a real component fixture from crates/wasm/fixtures/.
	//
	// To generate the fixture, build a Rust component targeting wasm32-wasip2
	// that exports `vane:plugin/registry@0.1.0` with a `get-metadata` function
	// returning a single l7-request export named "test-middleware". Then:
	//
	//   wasm-tools component new target/wasm32-wasip2/release/fixture.wasm \
	//       -o crates/wasm/fixtures/metadata_fixture.wasm
	//
	// This test is ignored because building the fixture requires wasm-tools and
	// a wasm32-wasip2 toolchain which are not part of the workspace dev-shell.
	#[ignore = "requires wasm-tools and wasm32-wasip2 toolchain; see doc comment to generate the fixture"]
	#[tokio::test]
	async fn smoke_load_metadata_fixture() {
		let fixture = Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/fixtures/metadata_fixture.wasm"));
		assert!(fixture.exists(), "fixture not found; see test doc comment to regenerate");
		...
		assert_eq!(meta.exports[0].name, "test-middleware");
		assert_eq!(meta.exports[0].kind, MiddlewareKind::L7Request);
	}
}
